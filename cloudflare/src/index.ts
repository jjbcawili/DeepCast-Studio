...
async function processJob(job:QueueJob,env:Env){
  ...
  if(job.kind==='plan'){
    try{
      ...
      const count=segmentCount(row.runtime); const prompt=`Create metadata and an outline for a ${row.format} podcast. Target runtime ${row.runtime} minutes. Topic: ${row.prompt}. Producer instructions: ${request.producerInstructions||'None'}. Source material: ${String(request.sourceMaterial||'').slice(0,16000)}. Current web-research brief: ${researchBrief.slice(0,10000)||'None'}. Return one JSON object only with: title (specific, polished, max 12 words), summary (2-3 concise sentences describing the finished episode, not instructions), and segments (an array of exactly ${count} concise segment focuses).`;
      const text=await generateText(env,prompt); let outline:string[]=[]; let generatedTitle=''; let generatedSummary=''; try{const p=JSON.parse(cleanJson(text)); if(p&&typeof p==='object'&&!Array.isArray(p)){generatedTitle=String(p.title||'').trim().slice(0,140);generatedSummary=String(p.summary||'').trim().slice(0,1200);if(Array.isArray(p.segments))outline=p.segments.map(String).slice(0,count);}}catch{} if(outline.length!==count)outline=Array.from({length:count},(_,i)=>`Segment ${i+1}: advance the requested Deep Dive with source-disciplined discussion.`);
      const nextRequest={...request,generatedSummary:generatedSummary||String(row.prompt).slice(0,1200)};
      ...
      await env.DB.prepare('UPDATE episodes SET title=?,request_json=?,expected_segments=?,updated_at=? WHERE id=?').bind(generatedTitle||row.title,JSON.stringify(nextRequest),count,now(),row.id).run();
  ...
  if(job.kind==='audio_segment'){
    ...
    try{
      await setEpisode(env,row.id,'SYNTHESIZING',Math.max(60,row.progress),`Dispatching audio segment ${i+1} of ${row.expected_segments} to the TTS compute runner.`);
...
async function handle(req:Request,env:Env){
  ...
  try{
    ...
    const jobPayload=url.pathname.match(/^\/internal\/audio-job\/([^/]+)\/([^/]+)$/);if(jobPayload&&req.method==='GET'){
      ...
      const host1=await hostWithVoiceReference(env,row.user_id,request.host1||{});const host2=await hostWithVoiceReference(env,row.user_id,request.host2||{});
